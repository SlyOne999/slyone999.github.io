# Shark Jack #
![alt text](/images/sharkjack.jpg?raw=true)
...
## Usage ## 
The device is built to slip into a pocket and execute automated tasks silently without requiring a laptop on-site:
### Standalone Battery Power ###
Features an integrated rechargeable battery, allowing it to boot up, run scripts, and harvest data entirely on its own power.

### Automated Payload Execution ### 
Utilises a simple switch to toggle between "Arming Mode" (to program scripts) and "Attack Mode" (to execute scripts immediately upon plugging into an active Ethernet port).

### Linux-Based Architecture ###
Runs a lightweight Linux operating system (OpenWrt), giving it access to standard, powerful network scanning command tools.
